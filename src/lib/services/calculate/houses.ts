import { degToRad, radToDeg, sanitizeDeg, trueObliquityDeg } from '../../utils';
...
import placidusIntermediateCusp, { placidusIntermediateCusps } from './placidusIntermediateCusp';

import placidusIntermediateCuspManual, { placidusIntermediateCusp as placidusIntermediateCuspDev } from './placidusIntermediateCusp';
...
export function calculateHouseSystem({
  ...
  switch (system.toLowerCase()) {
    ...
    case 'placidus':
    default:
      cusps = calculatePlacidusHouses(date, time, latitude, longitude);
      break;
  }
...
function printCuspComparison(myCusps) {
  for (let house = 1; house <= 12; house++) {
